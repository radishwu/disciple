...
abstract class ToastSubscriber<T>(context: Context?) : BaseSubscriber<T>(context) {

    override fun onOtherError(t: Throwable) {
        if (null != t.message) showErrorToast(t.message!!)
    }

    override fun onBusinessError(ex: BusinessException) {
        if (null != ex.msg) showErrorToast(ex.msg!!)
    }
